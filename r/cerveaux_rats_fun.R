...
cerveau_jfr <- function(jour,fonc,rat){
  ...
  d.filename <- sprintf("R%s/%s/%s/liste_R%s_%s_J%s.csv",rat,repertoires[[fonc]],fonc,rat,fonc,jour)
  ...
  for (slice.select in 1:length(day.slices[,1])){
    d.filename <- sprintf("R%s/%s/%s/%s-J%s-%s-bg-slice%i.txt",rat,repertoires[[fonc]],fonc,rat,jour,fonc,day.slices[slice.select,1])
...
FONC_3d_rat <- function(fonc,rat,val){
  ...
  for (j in 1:length(jours)){
    ...
    write.table(d, sprintf("R%s/%s/%s/%s-J%s-%s-bg-all.dat",rat,repertoires[[fonc]],fonc,rat,day,fonc), row.names=F, quote=F, sep='\t')
  }
}
...
rg_FONC_3d <- function(dim,fonc,rat,cl_bounds,opt,opt_2){
  ...
  for (jour in jours){
    day=jour#s[j]
    if (opt=='ker'){
      d <- read.table(sprintf("R%s/%s/%s/%s-J%s-%s-kertemp-all.dat",rat,repertoires[[fonc]],fonc,rat,day,fonc),header=T)
    }
    else if (opt=='dark'){
      d <- read.table(sprintf("R%s/%s/%s/%s-J%s-%s-dark-all.dat",rat,repertoires[[fonc]],fonc,rat,day,fonc),header=T)
    }
    else{
      d <- read.table(sprintf("R%s/%s/%s/%s-J%s-%s-bg-all.dat",rat,repertoires[[fonc]],fonc,rat,day,fonc),header=T)
    }
    ...
    if (opt_2=='pdf'){
      nom <- sprintf("R%s/%s/%s-J%s-%s-cerveau_clhist_clust.pdf",rat,"segmentation_manuelle",rat,jour,fonc)
      pdf(file=nom)
      #par(mfrow = c(2,2))
      m <- rbind(c(1,2),c(3,3))
      layout(m, heights = c(1,1), width=c(1,1), respect = TRUE)
...
seg_clust_3d <- function(fonc,rat,cl_bounds,cl_seg,hemi){
  ...
  for (jour in liste_j){
    ...
    t <- read.table(sprintf("R%s/%s/%s/%s-J%s-%s-bg-all.dat",rat,repertoires[[fonc]],fonc,rat,jour,fonc),header=T)
...
# Problème avec le .pdf qui n'accepte pas des largeurs ou longueurs trop émoignées de 7, le rendu est mauvais.
...
comp_2vs3d_clust <- function(fonc,rat,cl_bounds,num_tranche,opt){
  ...
  for (jour in liste_j){
    ...
    cerveau <- read.table(sprintf('R%s/%s/%s/%s-J%s-%s-bg-all.dat',rat,repertoires[[fonc]],fonc,rat,jour,fonc),header=T)
    ...
    if (opt==''){
      ...
      m <- rbind(c(1,2,3),c(4,0,5))
      layout(m, heights = c(1,1), width=c(1,1,1), respect = TRUE)
      ...
      pdf(file=nom,paper='A4r',height=7,width=7.1)
...
record_seg_cl <- function(rat, liste_clust, hemi){
  ...
  for (fonc in liste_fonc){
    ...
    for (jour in liste_j){
      ...
      write.table(d_seg, sprintf("R%s/%s/%s/isch3d-%s-%s-J%s.dat",rat,repertoires[[fonc]],fonc,fonc,rat,jour), row.names=F, quote=F, sep='\t')
    }
  ...
  for (fonc in liste_fonc){
    ...
    for (jour in jours){
      ...
      if (opt_2=='pdf'){
        ...
        nom <- sprintf("R%s/%s/%s-J%s-%s_clust_lesion.pdf",rat,"segmentation_manuelle",rat,jour,fonc)
        pdf(file=nom)
        ...
        scatterplot3d(seg.les.clust$x,
                      ...
                      main=paste("Mêmes rat et jour, ",fonc_seg)
        )
        title(sprintf("Zone lésée clusterisée : rat %s, modalité %s, jour %s",rat,fonc,jour),outer=TRUE)
        #close.screen( all = TRUE )
      }# lésion affichée
...
# ------------------- Représente les niveaux de gris pour chaque fonctionnalité, sur un cerveau segmenté avec FONC. Obsolète ? ------------------- #
...
gr_ngris_seg <- function(jour,FONC,rat){
  ...
  cerveau_seg <- read.table(sprintf("R%s/%s/%s/isch3d-%s-%s-J%s.dat",rat,repertoires[[FONC]],FONC,FONC,rat,jour),header=T)
...
# ------------------- Suivi temporel de tranches clusterisées ou segmentées. OBSOLETE. Répertoire fonctionnel_gris. Une représentation graphique par jour. ------------------- #
...
# Option 3 : chaîne. Indique le jour choisi pour l'hémisphère contralatéral, le plus souvent 00, 15 ou 22 suivant les rats.

# Option 4 : chaîne, sortie. pdf si 'pdf', affichage dans RStudio sinon.

dgris_temp_fonc <- function(rat,hemi,opt_1,opt_2,opt_3,opt_4){#liste_s_slice,
  ...
  if (any(opt_2=='cer')){# suivi sur le cerveau entier, boucle sur les jours existant pour chaque fonctionnalité.
    for (fonc in liste_fonc){
      ...
      for (jour in jours){# une fenêtre pour la fonctionnalité courante
        ...
        if (jour=="00"){
          # On utilise le jour choisi pour l'hémisphère contralatéral
          name_cerveau_hem <- sprintf("%s/%s-J%s-%s-bg-all.dat",fonc,rat,opt_3,fonc)
          cerveau_hem <- read.table(name_cerveau_hem,header=T)
          # On extrait la liste des valeurs de fonc pour cet hémisphère.
          cerveau_hem <- cerveau_hem[cerveau_seg$Label==2,]
          # On retire les valeurs manquantes.
          hem_sain <- cerveau_hem[,4]
          liste.nan <- is.na(hem_sain)
          hem_sain <- hem_sain[!liste.nan]
        }
        # on définit hem_sain au jour opt_3, on ne le modifie plus par la suite
        ...
        if (opt_4=='pdf'){
          ...
          if (length(isch)!=0){
            ...
            legend("topright", inset = 0.01, legend = c("Zone ischémiée",
    ...
    # fonctionnalité vue
  }# option 2 codée : plus grand volume disponible pour chaque jour. Contrairement aux deux autres possibilités, codée pour exploiter des cerveaux labélisés.
    ...
    for (fonc in liste_fonc){
      ...
      for (jour in jours){
        ...
        if (jour=="00"){
          # On utilise le jour choisi pour l'hémisphère contralatéral
          name_cerveau_hem <- sprintf("R%s/%s/%s/%s-J%s-%s-bg-all.dat",rat,repertoires[[fonc]],fonc,rat,opt_3,fonc)
          cerveau_hem <- read.table(name_cerveau_hem,header=T)
          # sélection de l'hémisphère sain : la fronctière est ici un segment de droite codé dans hemi
        ...
        }# On définit l'hémisphère contralatéral au jour choisi en option 3, on ne le modifie plus par la suite.
        
        cerveau_les <- cerveau_fonc # on va délimiter la partie lésée sur l'image observée, fonctionnalité et jour courants, segmentation mentionnée en option 2.
        block <- {
          if (fonc=='Anat'){# anatomique : trop de valeurs manquantes qui allongent la boucle
            list.nan <- is.na(cerveau_les[,4])
            cerveau_les <- cerveau_les[!list.nan,]
        ...
        # Les deux hémisphères : on sélectionne les tranches.
        block <- {
          l <- length(cerveau_fonc[,4])
          liste_tr <- rep(FALSE,l)
          for (tr in tranches){
...
ngris_box_fonc_cl_les <- function(rat,hemi,opt_1,opt_2,fonc_cl,lm_fonc,liste_s_slice,opt_3){
  ...
  # Liste des nombres de clusters de la lésion, par jour, avec la modalité fonc_cl.
  l_l_f_c <- liste_les_jfr_cl[[rat]]
  l_l_c <- l_l_f_c[[fonc_cl]]
  ...
  if (opt_2==''){
    ...
    if (opt_3=='pdf'){
      ...
      p <- p + geom_boxplot(outlier.shape = NA,varwidth = TRUE)
      p <- p + scale_fill_manual(values = alpha(c("red","orange"), .3))
      p <- p + ggtitle(bquote(atop(.(gg_title), atop(italic(.(subtitle)), "")))) + xlab("Jours") + ylab(fonc)
  ...
  # On passe aux représentations graphiques : diagrammmes en boîtes. Condition pour avoir des densités ? Option.
  for (fonc in liste_fonc){
...
ngris_box_clust <- function(rat, hemi, cl, lm_fonc, opt_1, liste_s_slice, opt_2){
  ...
  }# fonctionnalité vue, représentée graphiquement après clusterisation
}

# ------------------- Cartographie la distribution de valeurs d'une fonctionnalité, dans l'hémisphère sain, pour un rat. Option : pdf ou sortie RStudio . ------------------- #
