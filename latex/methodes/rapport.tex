...
Les bordures trac\'ees sur les images en CBF one \'et\'e transf\'er\'ees sur les autres images, ce qui a permis d'isoler les voxels correspondant \g{a} l'isch\'emie.

\par
Les valeurs prises, sur ces voxels, par ADC, BVf \dots T1map et VSI seront \'etudi\'ees \g{a} la sous-section suivante.
...
\par
En effet, m\^eme les cellules de l'h\'emisph\g{e}re contralat\'eral ont une activit\'e normale pendant et apr\g{e}s l'isch\'emie. %
...
il est possible que les donn\'ees dont on dispose correspondent \g{a} une r\'egion p\'eriph\'erique de la l\'esion.
...
C'est une id\'ee de Nicolas Glade (TIMC). Elle consiste \g{a} classer les voxels d'une image IRM en niveaux de gris. %
...
\\
Les classes de voxels ainsi obtenues correspondent \g{a} une r\'ealit\'e physiologique.
}
\label{exem_ADC_19}
\end{figure}
...
%
\item Les composantes d'une classification peuvent correspondre \g{a} des structures anatomiques comme nous l'avons vu dans le premier graphique de cette sous-section, %
...
%
\item La classification automatique, non supervis\'ee, regroupe des voxels correspondant peu ou prou \g{a} la partie isch\'emi\'ee et des voxels de l'h\'emisph\g{e}re contralat\'eral. %
...
\par
Toutefois, la classification des niveaux de gris, restreinte \g{a} l'image de la partie l\'es\'ee, donne des r\'esultats int\'eressants.
...
\par
On trouve des formes identiques, et un nombre important de voxels en commun, entre les clusters correspondant \g{a} un m\^eme jour et deux variables diff\'erentes %
...
\begin{itemize}
\item Les voxels pour lesquels le CBF, au jour 8, est inf\'erieur \g{a} 100 mL.min${}^{-1}$.(100g)${}^{-1}$ ;
\item Les voxels pour lesquels le CBF, au jour 8, est sup\'erieur \g{a} 100 mL.min${}^{-1}$.(100g)${}^{-1}$.
\end{itemize}
...
\FloatBarrier
\subsection{Mod\g{e}le ph\'enom\'enologique : \'etat d'un voxel en fonction du temps.}
...
\begin{description}
\item[Etat d'un voxel :] L'\'etat d'un voxel est enti\g{e}rement d\'etermin\'e par les valeurs prises, en ce voxel, des six variables ADC, BVf, CBF, CMRO2, SO2 et VSI ;
...
La premi\g{e}re hypoth\g{e}se simplificatice exclut une perturbation exog\g{e}ne de l'\'etat d'un voxel, %
...
D'ailleurs, les premi\g{e}res observations de cette exp\'erience sont effectu\'ees 30 minutes apr\g{e}s la reperfusion.
...
\begin{itemize}
\item Pour chaque voxel, la valeur de CBF \g{a} un temps $t$ -que l'on exprim\'era en jours- est donn\'ee par : %
...
\end{itemize}
%
On observe, sur la figure \ref{19_suivi_clust_les}, que les valeurs du CBF et de la CMRO2 sont r\'eparties de la m\^eme mani\g{e}re, %
aux jours 0 et 8.

\par
Afin de mod\'eliser l'\'evolution des valeurs de la CMRO2 en fonction du  temps, on utilise l'\'equation :
\begin{equation}
\der{CMRO2}=0,1\times\der{CBF}
\label{cbf_cmro2_l12}
\end{equation}
Le coefficient 0,1 est obtenu par changement d'\'echelle : $CBF\simeq 10\times CMRO2$ en g\'en\'eral.
%
De la m\^eme mani\g{e}re que pour CBF, on \'ecrit l'\'equation satisfaite par SO2 :
\begin{equation}
\der{SO2}=2,5\times (SO2_{asy1}-SO2)% \Ta
\label{so2_s_l2}
\end{equation}
Celle-ci ne variera pas, sauf pour la deri\g{e}re \'etape du deuxi\g{e}me r\'egime d'isch\'emie.
\begin{equation}
\der{CMRO2}=0,1\times \der{CBF}% \Ta
\end{equation}
...
\par
Les conditions initiales $SO2_{asy1}$ et $CMRO2_{trans}$ sont d\'efinies de la m\^eme mani\g{e}re que $CBF_{trans}$ : %
ce sont des distributions spatiales de CMRO2 et de SO2.

\par
Pour nos simulations, nous prendrons des \'echantillons gaussiens de lois respectives $\mathcal{N}(75,15)$ et $\mathcal{N}(6,7)$.
%
...
\begin{equation}
\der{CMRO2}=4, 6\times \left(CMRO2_{trans1}-CMRO2\right)% \Ta
\end{equation}
\end{comment}
...
Il concerne tous les voxels qui correspondent au jour 8, \g{a} des valeurs de CBF inf\'erieures \g{a} 100, et des valeurs de CMRO2 inf\'erieures \g{a} 10.
...
\label{cbf_is2}
\end{equation}
o\g{u} CBF${}_{trans1}$ est remplac\'ee par la variable spatiale CBF${}_{trans2}$. %
...
Ces \'equations concernent les voxels o\g{u} les valeurs de 100 ml.min${}^{-1}$.(100g)${}^{-1}$ pour le CBF, %
